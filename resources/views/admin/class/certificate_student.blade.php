                        ...
                        <table class="table table-bordered table-hover table-checkable" id="kt_datatable"
                            ...
                            @foreach($students as $key=>$item)
                                <tr>
                                    ...
                                    <td>
                                        @if($item->userDetails->has_certificate=='received')
                                            Авсан
                                        @else
                                            Аваагүй
                                        @endif
                                    ...
                                    <div class="modal fade" id="diplomaModal{{$item->id}}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form method="POST"
                                                          action="{{ route('admin.update-certificate', ['id' => $item->id]) }}">
                                                        @csrf
                                                        @method('PUT')
                                                        <label for="has_diploma">Сертификат</label>
                                                        <div class="input-group">
                                                            <select name="has_certificate" class="form-control">
                                                                <option
                                                                    value="received" {{ $item->userDetails->has_certificate === 'received' ? 'selected' : '' }}>
                                                                    Сертификат авсан
                                                                </option>
                                                                <option
                                                                    value="not_received" {{ $item->userDetails->has_certificate === 'not_received' ? 'selected' : '' }}>
                                                                    Сертификат аваагүй
                                                                </option>
                                                            </select>
                                                        </div>
                                                        <label for="diploma_received_date" class="mt-4">Сертификат олгосон өдөр</label>
                                                        <div class="input-group">
                                                            <div class="input-group">
